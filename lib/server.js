...
//
// 5. user: the User model instance related to this socket
//
// 6. device: the Device model instance related to this socket
//
// example:
//
// connections = {
...
function addUserContext(user, device, socket) {
    ...
    if (!item) {
        ...
        if (item[device.id])
            kick(item[device.id], proto.messages.KickNotify.Relogin);
...
function timeoutHandler(socket) {
    ...
    kick(socket, proto.messages.KickNotify.Timeout);
}
